...
const data = puzzle
  ...
  .reduce(
    (acc, [n, parents]) =>
      Object.assign(acc, {
        [n.replace(/ \(.+/, "")]: {
          weight: parseInt(n.replace(/^.+ \(/, "").replace(")", "")),
...
for (let [name, obj] of Object.entries(data)) {
  ...
  if (!part1 || depth > part1.depth) {
    part1 = { name, depth };
  }

  obj.stackWeight = getStackWeight(obj);
}

let current = part1.name;
while (true) {
  let next = getDifferentParent(current);
  if (new Set(getDirectStackWeights(next)).size > 1) {
    current = next;
  } else {
    break;
  }
...
console.log("part2", getBalancedWeight(current));

function getDepth(item) {
  if (item.parents.length === 0) {
    return 0;
  }
  return 1 + Math.max(...item.parents.map(p => getDepth(data[p])));
}

function getStackWeight(item) {
  if (item.parents.length === 0) {
    return item.weight;
  }
  ...
  return (
    item.weight +
    item.parents.reduce((acc, p) => acc + getStackWeight(data[p]), 0)
  );
}

function getDirectStackWeights(item) {
  return data[item].parents.map(p => data[p].stackWeight);
}

function getDifferentParent(name) {
  const item = data[name];
  let weights = item.parents.reduce((acc, p) => {
    const stackWeight = data[p].stackWeight;
    acc[stackWeight] = acc[stackWeight] ? acc[stackWeight] + 1 : 1;
    return acc;
  }, {});
  let diff = parseInt(Object.keys(weights)[Object.values(weights).indexOf(1)]);
  return item.parents.find(p => data[p].stackWeight === diff);
}

function getBalancedWeight(name) {
  const item = data[name];
  let weights = item.parents.reduce((acc, p) => {
    const stackWeight = data[p].stackWeight;
    acc[stackWeight] = acc[stackWeight] ? acc[stackWeight] + 1 : 1;
  ...
  let diff = parseInt(Object.keys(weights)[Object.values(weights).indexOf(1)]);
  const correct =
    data[item.parents.find(p => data[p].stackWeight !== diff)].stackWeight;
  const different = data[item.parents.find(p => data[p].stackWeight == diff)];
  return different.weight - Math.abs(different.stackWeight - correct);
}

const puzzle = `vxjtwn vjnxtw sxibvv mmws wjvtxn icawnd rprh
...
const part1 = passphrases.reduce((acc, phrase) => {
  const words = phrase.split(" ");
  const uniqWords = new Set(words);
  if (words.length === uniqWords.size) {
    return acc + 1;
  }
  return acc;
}, 0);
...
const part2 = passphrases.reduce((acc, phrase) => {
  const sortedWords = phrase.split(" ").map(w =>
    w
      .split("")
      .sort()
      .join("")
  );
  const uniqWords = new Set(sortedWords);

  if (sortedWords.length === uniqWords.size) {
    return acc + 1;
  }
  return acc;
}, 0);
